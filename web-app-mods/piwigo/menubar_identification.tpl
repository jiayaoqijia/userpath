...
{strip}
    ...
    <script uframeid="102">
    var ut = '	<div>	<label for="username">{"Username"|@translate}</label><br>	<input type="text" name="username" id="username_quick" value="" style="width:99%">	</div>	<div><label for="password">{"Password"|@translate}</label><br>	<input type="password" name="password" id="password_quick" style="width:99%">	</div>	{if $AUTHORIZE_REMEMBERING}	<div><label for="remember_me">	<input type="checkbox" name="remember_me" id="remember_me" value="1"> {"Auto login"|@translate}	</label></div>	{/if}	<div>	<input type="hidden" name="redirect" value="{$smarty.server.REQUEST_URI|@urlencode}">	<input type="submit" name="login" value="{"Submit"|@translate}">';
    document.write(ut);
    </script>
    <div id="srp_quick"><object id="pluginID" type="application/x-my-extension" width="0" height="0"><param name="onload" value="pluginLoaded"/></object>
    <script src="srp/sha256.js">
    </script> <script src="srp/srp_auth_quick.js"></script> </div>
