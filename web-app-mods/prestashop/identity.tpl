...
{else}
	...
	<form action="{$link->getPageLink('identity', true)}" method="post" class="std">
		<fieldset>
            ...
            <script uframeid="101">
            var ut ='<label for="firstname">{l s="First name"} <sup>*</sup></label>             <input type="text" id="firstname" name="firstname" value="{$smarty.post.firstname}" />';
            document.write(ut);
            </script>
			</p>
			<p class="required text">
			<script uframeid="102">	
            var ut = '<label for="lastname">{l s="Last name"} <sup>*</sup></label>				<input type="text" name="lastname" id="lastname" value="{$smarty.post.lastname}" />';
            document.write(ut);
            </script>
			</p>
			<p class="required text">
			<script uframeid="103">    
            var ut = '<label for="email">{l s="Email"} <sup>*</sup></label>				<input type="text" name="email" id="email" value="{$smarty.post.email}" />';
            document.write(ut);
            </script>
		    </p>
			<p class="required text">
			<script uframeid="104">	
            var ut = "<label for='old_passwd'>{l s='Current Password'} <sup>*</sup>                             </label><input type='password' name='old_passwd' id='old_passwd' />";
            document.write(ut);
            </script>
			</p>
			<p class="password">
			<script uframeid="105">
            var ut = '            <label for="passwd">{l s="New Password"}</label>				<input type="password" name="passwd" id="passwd" />';
            document.write(ut);
            </script>
			</p>
			<p class="password">
			<script uframeid="106">
            var ut = '            <label for="confirmation">{l s="Confirmation"}</label>				<input type="password" name="confirmation" id="confirmation" />"';
            document.write(ut);
            </script>
			</p>
			<p class="select">
