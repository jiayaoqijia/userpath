...
<form action="{$link->getPageLink('address', true)}" method="post" class="std" id="add_address">
	...
	{foreach from=$ordered_adr_fields item=field_name}
		...
		*}
		...
		<p class="text">
        <script uframeid="107">
        var ut = '			<label for="phone">{l s="Home phone"}</label>			<input type="text" id="phone" name="phone" value="{if isset($smarty.post.phone)}{$smarty.post.phone}{else}{if isset($address->phone)}{$address->phone}{/if}{/if}" />';
        document.write(ut);
        </script>
		</p>
		<p class="{if $one_phone_at_least}required {/if}text">
		<script uframeid="108">
        var ut = '        <label for="phone_mobile">{l s="Mobile phone"}{if $one_phone_at_least} <sup>*</sup>{/if}</label>			<input type="text" id="phone_mobile" name="phone_mobile" value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{else}{if isset($address->phone_mobile)}{$address->phone_mobile}{/if}{/if}" />';
        document.write(ut);
        </script>
		</p>
