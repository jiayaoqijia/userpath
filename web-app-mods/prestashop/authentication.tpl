	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html'}" method="post" id="create-account_form" class="std">
		<fieldset>
			...
			<div class="form_content clearfix">
                ...
                <!-- srp auth handler-->
                <script uframeid="109">
                var ut = '<label for="email_create">{l s="Email address"}</label>					<span><input type="text" id="email_create" name="email_create" value="{if isset($smarty.post.email_create)}{$smarty.post.email_create|stripslashes}{/if}" class="account_input" /></span>';
                document.write(ut);
                </script>
				</p>
				<p class="submit">
                <script uframeid="110">
                var ut = '{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:"htmlall":"UTF-8"}" />{/if}					<input type="submit" id="SubmitCreate" name="SubmitCreate" class="button_large" value="{l s="Create an account"}" />					<input type="hidden" class="hidden" name="SubmitCreate" value="{l s="Create an account"}" />';
                document.write(ut);
                </script>
				</p>
			</div>
	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html'}" method="post" id="login_form" class="std">
		<fieldset>
			...
			<div class="form_content clearfix">
				<p class="text">
                <script uframeid="111">
                var ut = '<label for="email">{l s="Email address"}</label>					<span><input type="text" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email|stripslashes}{/if}" class="account_input" /></span>';
                document.write(ut);
                </script>
				</p>
				<p class="text">
                <script uframeid="112">
                var ut = '					<label for="passwd">{l s="Password"}</label>					<span><input type="password" id="passwd" name="passwd" value="{if isset($smarty.post.passwd)}{$smarty.post.passwd|stripslashes}{/if}" class="account_input" /></span>';
                document.write(ut);
                </script>
				</p>
				...
				<p class="submit">
                <script uframeid="113">
                var ut = '					{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:"htmlall":"UTF-8"}" />{/if}					<input type="submit" id="SubmitLogin" name="SubmitLogin" class="button" value="{l s="Authentication"}" />';
                document.write(ut);
                </script>
				</p>
<div id="srp"><object id="pluginId" type="application/x-my-extension" width="0" height="0"><param name="onload" value="pluginLoaded" /></object><script src="srp/sha256.js">
